"use client";
import SectionTitle from "../Common/SectionTitle";
import OfferList from "./OfferList";
import PricingBox from "./PricingBox";

const Pricing = () => {
  return (
    <section id="pricing" className="relative z-10 py-16 md:py-20 lg:py-28">
      <div className="container">
        <SectionTitle
          title="Simple and Affordable Pricing"
          paragraph="We currently offer two classes, with flexible scheduling. Book a free trial today!"
          center
          width="665px"
        />

        <div className="w-3/4 mx-auto justify-center flex flex-row space-x-12">
          <PricingBox
            packageName="Standard"
            price="20"
            duration="hr"
            subtitle="Get help with any homework, and learn tips and tricks to ace your next test. Intended grade level: 9-12"
          >
            <OfferList text="Quality Teaching" status="active" />
            <OfferList text="Guaranteed Improvement" status="active" />
            <OfferList text="Available 24/7 for questions" status="active" />
          </PricingBox>
          <PricingBox
            packageName="Contest"
            price="30"
            duration="hr"
            subtitle="A rigorous and challenging course diving deep into competition mathematics. Intended grade level: 8-10"
          >
            <OfferList text="Quality Teaching" status="active" />
            <OfferList text="Guaranteed Improvement" status="active" />
            <OfferList text="Available 24/7 for questions" status="active" />
            <OfferList text="Counting and Probability" status="active" />
            <OfferList text="Algebra and Functions" status="active" />
            <OfferList text="Geometry" status="active" />
            <OfferList text="Number Theory" status="active" />
          </PricingBox>
        </div>
      </div>

      <div className="absolute bottom-0 left-0 z-[-1]">
        <svg
          width="239"
          height="601"
          viewBox="0 0 239 601"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <rect
            opacity="0.3"
            x="-184.451"
            y="600.973"
            width="196"
            height="541.607"
            rx="2"
            transform="rotate(-128.7 -184.451 600.973)"
            fill="url(#paint0_linear_93:235)"
          />
          <rect
            opacity="0.3"
            x="-188.201"
            y="385.272"
            width="59.7544"
            height="541.607"
            rx="2"
            transform="rotate(-128.7 -188.201 385.272)"
            fill="url(#paint1_linear_93:235)"
          />
          <defs>
            <linearGradient
              id="paint0_linear_93:235"
              x1="-90.1184"
              y1="420.414"
              x2="-90.1184"
              y2="1131.65"
              gradientUnits="userSpaceOnUse"
            >
              <stop stopColor="#4A6CF7" />
              <stop offset="1" stopColor="#4A6CF7" stopOpacity="0" />
            </linearGradient>
            <linearGradient
              id="paint1_linear_93:235"
              x1="-159.441"
              y1="204.714"
              x2="-159.441"
              y2="915.952"
              gradientUnits="userSpaceOnUse"
            >
              <stop stopColor="#4A6CF7" />
              <stop offset="1" stopColor="#4A6CF7" stopOpacity="0" />
            </linearGradient>
          </defs>
        </svg>
      </div>
    </section>
  );
};

export default Pricing;
